import {MenuAction, SettingsMenu} from "../../pkg";
...
const settings_menu = Object.values(SettingsMenu).filter(x => !isNaN(Number(x))).sort() as number[]

export function SettingsMenuView({position, consume_action}: SettingsMenuViewProps) {
...
export const setup_settings_menu_view: ViewSetup<typeof render_settings_menu> = (setView) => (position) => {
    setView(view => {
        let other_view = view[0] !== SettingsMenuView;
        ...
        if (other_view || props_changed) {
            ...
            return [SettingsMenuView as FunctionComponent, {
                position,
                consume_action: set_main_menu_action
            }]
        }
